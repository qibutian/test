...
public class NetJSonRecycleAdapter extends RecyclerBaseAdapter<JSONObject>
		implements INetAdapter {
	...
	public NetJSonRecycleAdapter(String api, Context context, int mResource) {
	...
	@Override
	public void bindView(View itemV, BaseViewHolder holder, int position,
			JSONObject jo) {
		for (Iterator<FieldMap> iterator = fields.iterator(); iterator
			...
			v = holder.get(fieldMap.getRefId());
	...
	@Override
	public net.duohuo.dhroid.reclcleadapter.RecyclerBaseAdapter.BaseViewHolder buildNormalHolder(
			int viewType, ViewGroup parent) {

		BaseViewHolder holder = new BaseViewHolder(mInflater.from(mContext)
